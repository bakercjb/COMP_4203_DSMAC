...
void DSMAC::startup()
{
	...
	declareOutput("DSMAC packet breakdown");
}

void DSMAC::timerFiredCallback(int timer)
{
	switch (timer) {
...
void DSMAC::handleCarrierSenseResult(int returnCode)
{
	switch (returnCode) {
...
void DSMAC::fromNetworkLayer(cPacket * netPkt, int destination)
{
	DSMACPacket *macPkt = new DSMACPacket("DSMAC data packet", MAC_LAYER_PACKET);
	...
	collectOutput("DSMAC packet breakdown", "Received from App");
	...
	if (bufferPacket(macPkt)) {
		 ...
		 */
		collectOutput("DSMAC packet breakdown", "Overflown");
		trace() << "WARNING DSMAC buffer overflow";
	}
}

void DSMAC::attemptTx()
{
...
void DSMAC::sendBeaconsOrData()
{

	if (remainingBeaconsToTx > 0) {
		remainingBeaconsToTx--;

		DSMACPacket *beaconFrame =
			new DSMACPacket("DSMAC beacon packet", MAC_LAYER_PACKET);
		...
		collectOutput("DSMAC packet breakdown", "sent beacons");
		...
		if (numTxTries == numTx){
			...
			collectOutput("DSMAC packet breakdown", "sent data pkts");
		}
		else{
			trace() << "Sending copy of data packet";
			collectOutput("DSMAC packet breakdown", "copies of sent data pkts");
		}
...
{
	DSMACPacket *macFrame = dynamic_cast <DSMACPacket*>(pkt);
	if (macFrame == NULL){
		collectOutput("DSMAC packet breakdown", "filtered, other MAC");
		return;
	}

	int destination = macFrame->getDestination();
	if (destination != SELF_MAC_ADDRESS && destination != BROADCAST_MAC_ADDRESS){
		collectOutput("DSMAC packet breakdown", "filtered, other dest");
		return;
	}
	...
	switch (macFrame->getFrameType()) {
		...
		case BEACON_FRAME:{
			collectOutput("DSMAC packet breakdown", "received beacons");
			if (macState == MAC_STATE_DEFAULT) {
				...
				collectOutput("DSMAC packet breakdown", "received beacons, ignored");
				break;
			}
			...
			if ((dutyCycle > 0.0) && (dutyCycle < 1.0)) {
				 ...
				 * another one does not. DSMAC was not designed for
		...
		case DATA_FRAME:{
			...
			collectOutput("DSMAC packet breakdown", "received data pkts");
		...
		default:{
			...
			collectOutput("DSMAC packet breakdown", "unrecognized type");
		}
	}
}

int DSMAC::handleControlCommand(cMessage * msg)
{
	DSMACControlCommand *cmd = check_and_cast <DSMACControlCommand*>(msg);

	switch (cmd->getDSMACCommandKind()) {
		...
		case SET_LISTEN_INTERVAL:{
			...
			if (tmpValue < 0.0)
				trace() << "WARNING: invalid listen interval value sent to DSMAC";
			else {
		...
		case SET_BEACON_INTERVAL_FRACTION:{
			...
			if ((tmpValue < 0.0) || (tmpValue > 1.0))
				trace() << "WARNING: invalid Beacon Interval Fraction value sent to DSMAC";
			else
		...
		case SET_PROB_TX:{
			...
			double tmpValue = cmd->getParameter();
			if ((tmpValue < 0.0) || (tmpValue > 1.0))
				trace() << "WARNING: invalid ProbTX value sent to DSMAC";
			else
				probTx = tmpValue;
			break;
		}
		...
		case SET_NUM_TX:{
			...
			if (tmpValue < 0 || tmpValue - ceil(tmpValue) != 0)
				trace() << "WARNING: invalid NumTX value sent to DSMAC";
			else
				numTx = (int)tmpValue;
			break;
		}
		...
		case SET_RANDOM_TX_OFFSET:{
			...
			double tmpValue = cmd->getParameter() / 1000.0;
			if (tmpValue <= 0.0)
				trace() << "WARNING: invalid randomTxOffset value sent to DSMAC";
			else
		...
		case SET_RETX_INTERVAL:{
			...
			double tmpValue = cmd->getParameter() / 1000.0;
			if (tmpValue <= 0.0)
				trace() << "WARNING: invalid reTxInterval value sent to DSMAC";
			else
		...
		case SET_BACKOFF_TYPE:{
			...
			if (tmpValue == 0.0) {
				if ((dutyCycle > 0.0) && (dutyCycle < 1.0))
					...
					trace() << "WARNING: invalid backoffType value sent to DSMAC. Backoff timer = sleeping interval, but sleeping interval is not defined because duty cycle is zero, one, or invalid.";
			}
				...
				trace() << "WARNING: invalid backoffType value sent to DSMAC";
		...
		case SET_BACKOFF_BASE_VALUE:{
			...
			double tmpValue = cmd->getParameter() / 1000.0;
			if (tmpValue < 0)
				trace() << "WARNING: invalid backoffBaseValue sent to DSMAC";
			else
